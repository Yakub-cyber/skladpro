...
const PRODUCTS = P.map((r, i) => ({
  ...
  // Партионный учёт: демо стартует с единственной партией по seed-cost на
  // весь начальный остаток (Unix epoch — заведомо раньше любого нового
  // прихода, поэтому первой уйдёт при FIFO-списании).
  batches: r[6] > 0
